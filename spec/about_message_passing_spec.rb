...
describe 'Message Passing' do
  ...
  it 'should demonstrate methods_can_be_invoked_more_dynamically' do
    ...
    expect(mc.send('caught?')).to be_true
    expect(mc.send('caught' + __)).to be_truthy # What do you need to add to the first string?
    expect(mc.send('CAUGHT?'.____)).to be_truthy # What would you need to do to the string?
  end
